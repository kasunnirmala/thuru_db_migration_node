...
async function connect() {
  try {
    ...
    let result = await connection.awaitQuery(
      `SELECT * FROM forum_post_likes fpl, app_users au WHERE fpl.liked_by_id=au.id`
    );
    ...
    var post = await thuru_new_db
      ...
      if (user && post) {
        const result = await postLikes.insertOne({
          thisUUID: uniqid("postlikes"),
          likedBy: {
            $ref: "User",
            $id: user._id,
          },
          likedByUUID: user.thisUUID,
          post: {
            $ref: "Post",
            $id: post._id,
          },
          ...
          isActive: true,
          created_date: new Date(el.dateTime),
        });
      }
      console.log(i++);
    }
